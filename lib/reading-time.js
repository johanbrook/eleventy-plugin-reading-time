module.exports = function readingTime(
  postOrContent,
  { printSeconds = false, raw = false, speed = 300, minuteLabel, secondLabel } = {}
) {
  const htmlContent =
  ...
  if (printSeconds === true) {
    ...
    if (raw === false) {
      const mins = min + ' ' + (minuteLabel || 'minute') + (min === 1 ? '' : 's') + ', ';
      const secs = sec + ' ' + (secondLabel || 'second') + (sec === 1 ? '' : 's');
    ...
    if (raw === false) {
      est = min + ' ' + (minuteLabel ? minuteLabel + (min === 1 ? '' : 's') : 'min');
    } else {
      est = min;
    }
  }
